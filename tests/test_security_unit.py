...
async def test_get_current_user_raises_exception_on_expired_token(session_override):
    ...
    token = create_access_token({"sub": "johndoe"}, expires_delta=timedelta(seconds=-1))
    with pytest.raises(HTTPException) as ex:
        await get_current_user(token, session_override)
    assert ex.value.status_code == 401
    assert "could not validate user credentials" in str(ex.value.detail).lower()


@pytest.mark.asyncio(loop_scope="session")
async def test_get_current_user_raises_exception_on_invalid_user(
