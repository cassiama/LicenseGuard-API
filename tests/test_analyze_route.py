...
# make sure PYTHONPATH=src when running pytest
...
def _post_file(filename: str, data: bytes, content_type: str = "text/plain"):
    # httpx-style multipart upload (filename, fileobj, content-type)
    files = {"file": (filename, io.BytesIO(data), content_type)}
    return client.post("/analyze", files=files)


def test_accepts_valid_requirements_txt_minimal():
    resp = _post_file(
        "requirements.txt",
        b"requests==2.32.3\nfastapi>=0.110\n",
        "text/plain",
...
def test_accepts_valid_with_charset_param_on_content_type():
    resp = _post_file(
        "requirements.txt",
...
def test_rejects_wrong_media_type_even_if_txt_extension():
    resp = _post_file(
        "requirements.txt",
        b"anyio>=4.0.0",
...
def test_rejects_wrong_extension_even_if_media_type_ok():
    resp = _post_file(
...
def test_rejects_empty_file():
    resp = _post_file("requirements.txt", b"", "text/plain")
    assert resp.status_code == 400
    assert "empty" in resp.text.lower()


def test_rejects_comments_only():
    content = b"# just a comment\n   \n# another comment"
    resp = _post_file("requirements.txt", content, "text/plain")
    assert resp.status_code == 422
    assert "no requirements" in resp.text.lower()


def test_rejects_invalid_requirement_lines_single():
...
def test_rejects_invalid_requirement_lines_mixed_list():
...
def test_missing_file_param_triggers_422_from_fastapi():
    # if no 'files' field is passed in, then FastAPI's validation layer should automatically throw a HTTP 422
    resp = client.post("/analyze")
    assert resp.status_code == 422


def test_non_txt_filename_but_correct_content_type():
    # a client could lie about the extension even if the MIME type is correct
    resp = _post_file("image.png", b"requests==2.32.3\n", "text/plain")
    assert resp.status_code == 422


def test_accepts_file_with_editable_and_named_entries():
...
def test_accepts_file_with_include_and_named_entries():
...
def test_accepts_pep508_direct_url_line():
